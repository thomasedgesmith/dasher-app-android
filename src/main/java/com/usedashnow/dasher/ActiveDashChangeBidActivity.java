...
public class ActiveDashChangeBidActivity extends AppCompatActivity implements TurbolinksAdapter {
    ...
    public void visitProposedToLocationWithAction(String location, String action) {
        ...
        if (location.contains("messages")) {
            ...
            this.finish();
        } else if (location.contains("change_bid")) {
            // do nothing for now. Since we're just on the same activity
        } else if (location.contains("bids")) {
            intent = new Intent(this, ActiveDashChangeBidActivity.class);
            intent.putExtra(INTENT_URL, location);
            intent.putExtra(DASH_ID, dashID);
            intent.putExtra(RESOURCE_ID, resourceID);
